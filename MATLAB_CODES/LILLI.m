...
fontSize = 8;
waveLength = 0.525e-6; % Green LED
WaveNumber = 2*pi/waveLength;
nDome = 1;%1.45;

%:----- Dome
NoRing =  9;                             % No. of rings.
...
LedHeight =17e-3%16.6e-3;                           % New sample position
...
background = strcat('background.tiff');
B = double(imread(background));
...
for j1 = 1:Nled           
 ...
 Beta = 1;
  ...
  if   j1<170
  Iccd = Beta*1*double(imread(Path));
  Iccd = Iccd - B;
  end
% % %   Iccd = double(imread(Path));
%   if  61<j1&& j1<92
%   Iccd = Beta*(4/7)*double(imread(Path));
%   end
%    if  91<j1&& j1<128
%   Iccd = Beta*(4/10)*double(imread(Path));
%    end
%   if  127<j1&& j1<170
%   Iccd = Beta*(300/600)*double(imread(Path));
%   end
%   
  if  169<j1&& j1<272
  Iccd = Beta*double(imread(Path));
  Iccd = Iccd - B;
  end
...
figure;imagesc(oxaxis*1e3,oyaxis*1e3,(abs(objectRecoverHH)));
colormap gray;colorbar;xlabel('x-axis mm');ylabel('y-axis mm');
axis image;
...
figure;
imagesc(oxaxis*1e3,oyaxis*1e3,(abs(objectRecoverHH)));
colormap gray;colorbar;
axis image;xlabel('x-axis mm');ylabel('y-axis mm');
title('Recovered amplitude,0 deg', 'FontSize', fontSize, 'FontName', 'calibri')

figure;
imagesc(oxaxis*1e3,oyaxis*1e3,(angle(objectRecoverHH)));
colormap gray;colorbar; axis image;xlabel('x-axis mm');ylabel('y-axis mm');
title('Recovered phase,0 deg', 'FontSize', fontSize, 'FontName', 'calibri')

figure;
imagesc(kx2*1e-3,ky2*1e-3, log(abs(objectRecoverFTHH).*CTF_synth));axis image;
title('Fourier Transform of reccovered object 0 deg', 'FontSize', fontSize, 'FontName', 'calibri')
colormap default;colorbar; xlabel('f_x, rad/mm');ylabel('f_y, rad/mm');

figure;
imagesc(kx1*1e-3,ky1*1e-3,(abs(pupil.*CTF))*1e-3);colorbar; axis image;
title('Amplitude of CTF_0 deg');xlabel('kx, rad/mm');ylabel('ky, rad/mm');

figure;
imagesc(kx1*1e-3,ky1*1e-3,(angle(pupil.*CTF))*1e3);colorbar; axis image;
title('phase of CTF_0 deg');xlabel('kx, rad/mm');ylabel('ky, rad/mm');
 pause
...
imagesc(oxaxis*1e3,oyaxis*1e3,(abs(objectRecoverHH)));
colormap gray;colorbar;
axis image;axis image;xlabel('x-axis mm');ylabel('y-axis mm');
title('Recovered amplitude,0 deg', 'FontSize', fontSize, 'FontName', 'calibri')

figure;
imagesc(oxaxis*1e3,oyaxis*1e3,(angle(objectRecoverHH)));
colormap gray;colorbar; axis image;axis image;xlabel('x-axis mm');ylabel('y-axis mm');
title('Recovered phase,0 deg', 'FontSize', fontSize, 'FontName', 'calibri')
...
figure;
imagesc(oxaxis*1e3,oyaxis*1e3,(abs(objectRecoverHV)));
colormap gray;colorbar;
axis image;axis image;xlabel('x-axis mm');ylabel('y-axis mm');
title('Recovered amplitude,0 deg', 'FontSize', fontSize, 'FontName', 'calibri')

figure;
imagesc(oxaxis*1e3,oyaxis*1e3,(angle(objectRecoverHV)));
colormap gray;colorbar; axis image;axis image;xlabel('x-axis mm');ylabel('y-axis mm');
title('Recovered phase,0 deg', 'FontSize', fontSize, 'FontName', 'calibri')

figure;
imagesc(kx2*1e-3,ky2*1e-3,log(abs(objectRecoverFTHV).*CTF_synth));axis image;
title('Fourier Transform of reccovered object 0 deg', 'FontSize', fontSize, 'FontName', 'calibri')
colormap default;colorbar; axis image;xlabel('kx, rad/mm');ylabel('ky, rad/mm');

figure;
imagesc(kx1*1e-3,ky1*1e-3,(abs(pupil.*CTF)));colorbar; axis image;
title('Amplitude of CTF_45 deg');xlabel('kx, rad/mm');ylabel('ky, rad/mm');

figure;
imagesc(kx1*1e-3,ky1*1e-3,(angle(pupil.*CTF)));colorbar; axis image;
title('phase of CTF_45 deg');xlabel('kx, rad/mm');ylabel('ky, rad/mm');
...
figure;
imagesc(oxaxis*1e3,oyaxis*1e3,(abs(objectRecoverVH)));
colormap gray;colorbar;
axis image;xlabel('y-axis mm');ylabel('x-axis mm');
title('Recovered amplitude,0 deg', 'FontSize', fontSize, 'FontName', 'calibri')

figure;
imagesc(oxaxis*1e3,oyaxis*1e3,(angle(objectRecoverVH)));
colormap gray;colorbar; axis image;xlabel('y-axis mm');ylabel('x-axis mm');
title('Recovered phase,0 deg', 'FontSize', fontSize, 'FontName', 'calibri')

figure;
imagesc(kx2*1e-3,ky2*1e-3,log(abs(objectRecoverFTVH).*CTF_synth));axis image;
title('Fourier Transform of reccovered object 0 deg', 'FontSize', fontSize, 'FontName', 'calibri')
colormap default;colorbar; xlabel('kx, rad/mm');ylabel('ky, rad/mm');

figure;
imagesc(kx1*1e-3,ky1*1e-3,(abs(pupil.*CTF)));colorbar; axis image;
title('Amplitude of CTF_90 deg');xlabel('kx, rad/mm');ylabel('ky, rad/mm');

figure;
imagesc(kx1*1e-3,ky1*1e-3,(angle(pupil.*CTF)));colorbar; axis image;
title('phase of CTF_90 deg');xlabel('kx, rad/mm');ylabel('ky, rad/mm');
...
figure;
imagesc(oxaxis*1e3,oyaxis*1e3,(abs(objectRecoverVV)));
colormap gray,colorbar;
axis image;xlabel('y-axis mm');ylabel('x-axis mm');
title('Recovered amplitude,135 deg', 'FontSize', fontSize, 'FontName', 'calibri')

figure;
imagesc(oxaxis*1e3,oyaxis*1e3,(angle(objectRecoverVV)));
colormap gray,colorbar; axis image;xlabel('y-axis mm');ylabel('x-axis mm');
title('Recovered phase,135 deg', 'FontSize', fontSize, 'FontName', 'calibri')

figure;
imagesc(kx2*1e-3,ky2*1e-3,log(abs(objectRecoverFTVV).*CTF_synth));axis image;
title('Fourier Transform of reccovered object 135 deg', 'FontSize', fontSize, 'FontName', 'calibri')
colormap default,colorbar; xlabel('kx, rad/mm');ylabel('ky, rad/mm');

figure;
imagesc(kx1*1e-3,ky1*1e-3,(abs(pupil.*CTF)));colorbar; axis image;
title('Amplitude of CTF_135 deg');xlabel('kx, rad/mm');ylabel('ky, rad/mm');

figure;
imagesc(kx1*1e-3,ky1*1e-3,(angle(pupil.*CTF)));colorbar; axis image;
title('phase of CTF_135 deg');xlabel('kx, rad/mm');ylabel('ky, rad/mm');
...
title('Recovered phase, 135 deg', 'FontSize', fontSize, 'FontName', 'calibri')
colormap default,colorbar; xlabel('y-axis mm');ylabel('x-axis mm');
...
theta_recover = theta_recover*180/pi;
...
caxis([0 1.2])
figure;imagesc(abs(theta_recover)); axis image;title('recovered theta,rad');
colorMap = hsv(256);
colormap(colorMap);   % Apply the colormap
colorbar;
caxis([0 180])
